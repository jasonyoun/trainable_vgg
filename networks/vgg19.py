...
class VGG19:
	...
	def __init__(self, num_classes=None, npy_path=None,
		...
		Inputs:
			...
			- npy_path: path of the vgg19 pre-trained weights in .npy format
	...
	def build_partial(self, rgb, build_until, train_mode=None):
		"""
		Build partial VGG-19 network.
		For example, one can call this to build a VGG-19 without FC layers.
		...
		conv3_4 = self.nc.conv_layer(conv3_3, 3, 256, 256, 'conv3_4')
		if build_until is 'conv3_4': return conv3_4
		pool3 = self.nc.max_pool(conv3_4, 'pool3')
		...
		conv4_4 = self.nc.conv_layer(conv4_3, 3, 512, 512, 'conv4_4')
		if build_until is 'conv4_4': return conv4_4
		pool4 = self.nc.max_pool(conv4_4, 'pool4')
		...
		conv5_4 = self.nc.conv_layer(conv5_3, 3, 512, 512, 'conv5_4')
		if build_until is 'conv5_4': return conv5_4
		pool5 = self.nc.max_pool(conv5_4, 'pool5')
